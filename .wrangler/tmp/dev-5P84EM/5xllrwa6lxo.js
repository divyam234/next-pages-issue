// .wrangler/tmp/bundle-Eh0j8C/checked-fetch.js
...
// .wrangler/tmp/pages-0aIsqv/bundledWorker-0.5730829053145532.mjs
...
var Z = A((O) => {
  ...
  O.parse = ge;
  O.serialize = ye;
  var me = Object.prototype.toString, E = /^[\u0009\u0020-\u007e\u0080-\u00ff]+$/;
  function ge(t, e) {
    if (typeof t != "string")
      throw new TypeError("argument str must be a string");
    for (var r = {}, s = e || {}, a = s.decode || we, n = 0; n < t.length; ) {
      var i = t.indexOf("=", n);
      if (i === -1)
        break;
      var o = t.indexOf(";", n);
  ...
  async set(e, r) {
    ...
    switch (await this.update(e, JSON.stringify(s)), s.value?.kind) {
      case "FETCH": {
        ...
        await this.setTags(a, { cacheKey: e }), q(a).forEach((n) => M.delete(n));
      }
    }
  }
  async get(e) {
    ...
    switch (s.value?.kind) {
      case "FETCH":
        return await this.loadTagsManifest(), q(s.value.data.tags ?? []).some((i) => {
          if (M.has(i))
            return true;
...
function S(t, e, r) {
  ...
  let s = r ? "" : "i", a = [];
  return { match: (0, j.default)(`%${t}%${s}`, a).exec(e), captureGroupKeys: a };
}
...
function V(t, { url: e, cookies: r, headers: s, routeDest: a }) {
  switch (t.type) {
    case "host":
...
function T(t, e, r) {
  let { match: s, captureGroupKeys: a } = S(t, e);
...
async function C(t, { request: e, assetsFetcher: r, ctx: s }, { path: a, searchParams: n }) {
  ...
  try {
    switch (t?.type) {
      ...
      case "override": {
        i = P(await r.fetch(L(c, t.path ?? a))), t.headers && g(i.headers, t.headers);
        break;
      }
      case "static": {
        i = await r.fetch(L(c, a));
        break;
      }
      default:
...
var _ = class {
  ...
  locales;
  checkRouteMatch(e, r) {
    let s = S(e.src, this.path, e.caseSensitive);
    ...
    }) && !e.missing?.find((n) => V(n, a).valid) && !(r && e.status !== this.status))
  ...
  applyLocaleRedirects(e) {
    if (!e.locale?.redirect || (this.locales || (this.locales = {}), Object.assign(this.locales, e.locale.redirect), !/^\^(.)*$/.test(e.src) && e.src !== this.path) || this.headers.normal.has("location"))
      return;
    let { locale: { redirect: s, cookie: a } } = e, n = a && this.cookies[a], i = N(n ?? ""), o = N(this.reqCtx.request.headers.get("accept-language") ?? ""), d = [...i, ...o].map((m) => s[m]).filter(Boolean)[0];
    if (d) {
...
async function Y(t, e, r) {
  let s = new _(e.routes, r, t, e.wildcard), a = await X(s);
  return Pe(t, a, r);
}
...
async function Pe(t, { path: e = "/404", status: r, headers: s, searchParams: a, body: n }, i) {
...
// .wrangler/tmp/pages-0aIsqv/5xllrwa6lxo.js
...
// .wrangler/tmp/bundle-Eh0j8C/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-Eh0j8C/middleware-loader.entry.ts
